...
어떤 작업을 도와드릴까요? 이 스키마는 금융 거래와 관련된 다양한 엔드포인트와 데이터 구조를 정의하고 있습니다.

**궁금한 점이나 필요한 작업이 있으신가요? 예를 들어:**

1. **특정 엔드포인트에 대한 설명 요청:** (예: "어떻게 주문을 넣나요?")
2. **특정 데이터 필드에 대한 질문:** (예: "여기서 `clientOrderId`는 무엇을 의미하나요?")
3. **여러 개념을 비교 요청:** (예: "일반 주문과 조건부 주문의 차이점은 무엇인가요?")
4. **특정 시나리오에 따른 API 호출 예시 요청:** (예: "주가 하락 시 자동으로 매수 주문을 넣으려면 어떤 API를 사용해야 하나요?")
