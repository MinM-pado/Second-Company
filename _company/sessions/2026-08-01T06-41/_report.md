...
> 📋 **지시**: {
  ...
  "tags": [
    ...
    {
      "name": "Ranking",
      "description": "국내·미국 주식의 랭킹을 조회하는 그룹입니다. 시장 전체 거래대금·거래량 상위, 급상승·급하락(등락률), 토스증권 체결 기준 거래대금·거래량 상위 랭킹을 실시간부터 1년까지 거래일 기준 기간별로 제공합니다. 순위와 함께 현재가·기준가·등락률·거래량·거래대금을 응답하며, 계좌와 무관한 정보로 토큰만으로 호출할 수 있습니다."
    },
    {
      "name": "Market Indicators",
      "description": "국내 주요 지수(코스피·코스닥)와 한국 국채 금리 시세, KRX 투자자별 매매대금을 조회하는 그룹입니다. 투자자별 매매대금은\n코스피·코스닥을 대상으로 개인·외국인·기관·기타법인 분류(기관 세부 분류 포함)의 매수·매도 거래대금을\n제공합니다. 계좌와 무관한 정보로 토큰만으로 호출 가능합니다.\n\n이 그룹의 모든 엔드포인트는 아래 **심볼 카탈로그**의 8종 심볼만 지원하며, 카탈로그에 없는 심볼은 400 `unsupported-symbol` 로 응답합니다.\n개별 종목(예: `005930`, `AAPL`)의 시세는 본 그룹이 아닌 Market Data 그룹(`GET /api/v1/prices`, `/candles`)을 사용하세요.\n\n#### 심볼 카탈로그\n\n| 심볼 | 명칭 | 자산 유형 | 표시 단위 | 비고 |\n|------|------|----------|----------|------|\n| `KOSPI` | 코스피 | `INDEX` | 포인트 | 투자자별 매매대금 지원 |\n| `KOSDAQ` | 코스닥 | `INDEX` | 포인트 | 투자자별 매매대금 지원 |\n| `KR_BOND_2Y` | 한국 국채 2년 | `BOND` | % | 수익률(금리), 예: `3.25` = 3.25% |\n| `KR_BOND_3Y` | 한국 국채 3년 | `BOND` | % | |\n| `KR_BOND_5Y` | 한국 국채 5년 | `BOND` | % | |\n| `KR_BOND_10Y` | 한국 국채 10년 | `BOND` | % | |\n| `KR_BOND_20Y` | 한국 국채 20년 | `BOND` | % | |\n| `KR_BOND_30Y` | 한국 국채 30년 | `BOND` | % | |\n\n> 캔들 차트(`/candles`)의 분봉(`1m`)은 지수(`KOSPI`·`KOSDAQ`)만 지원합니다. 국채(`KR_BOND_*`)는 일봉(`1d`)만 지원하며, 분봉 요청 시 400 `invalid-request` 로 응답합니다."
    },
    {
      "name": "Account",
    ...
    },
    {
      ...
      "description": "특정 종목의 가격을 감시해 조건 충족 시 자동으로 매매(매수/매도) 주문을 생성하는 조건주문을 등록·수정·취소하는 그룹입니다. 단일(SINGLE), OCO(One-Cancels-the-Other, 하나의 조건 충족 시 나머지 조건 자동 취소), OTO(One-Triggers-the-Other, 첫번째 조건 체결 후 두번째 조건 감시 시작) 조건주문을 지원합니다. 계좌에 직접 영향을 주는 API이므로 액세스 토큰과 `X-Tossinvest-Account` 헤더가 모두 필요합니다."
    },
    {
      "name": "Conditional Order History",
      "description": "등록한 조건주문의 목록과 상세를 조회하는 그룹입니다. `status` 파라미터로 진행 중(`OPEN`)·종료(`CLOSED`)를 선택해 조회하며, 이 API 로 등록한 조건주문뿐 아니라 다른 채널(토스증권 앱 등)에서 등록한 조건주문도 함께 반환됩니다. 호출 시 `X-Tossinvest-Account` 헤더가 필요합니다."
    },
    {
      "name": "Order History",
      "description": "제출한 주문의 처리 상태와 체결 내역을 조회하는 그룹입니다. `status` 파라미터로 라이프사이클 그룹(현재 진행 중 주문 `OPEN`, 종료된 주문 `CLOSED`)을 선택해 주문 목록을 조회하고, 개별 `orderId`로 모든 상태의 주문 상세를 조회할 수 있습니다. 상세 조회는 부분 체결 내역 등 주문 단위 정보를 포함합니다. 호출 시 `X-Tossinvest-Account` 헤더가 필요합니다."
    },
    {
      "name": "Order Info",
              ...
              "application/json": {
                ...
                "examples": {
                  ...
                  "fractionalQuantityOutsideRegularHours": {
                    "summary": "정규장 외 시간 소수점 수량 주문 불가",
                    "value": {
                      "error": {
                        "requestId": "01HXYZABCDEFG123456789",
                        "code": "fractional-quantity-outside-regular-hours",
                        "message": "미국 주식 소수점 수량 주문은 정규장 시간에만 접수할 수 있습니다.",
                        "data": {
                          "field": "quantity",
                          "regularHours": {
                            "start": "2026-03-30T09:30:00-04:00",
...
> 💡 **핵심 산출**: 어떤 작업을 도와드릴까요? 이 스키마는 금융 거래와 관련된 다양한 엔드포인트와 데이터 구조를 정의하고 있습니다.
> 📝 산출물 길이: 364자
...
1. **필요한 정보:** 
    *  `schema` 문의: API 내부 데이터 구조와 특정 항목에 대한 설명이 필요합니다. 
    *  "브리프":  API 연동, 주문 생성 등의 과정을 담은 간략한 설명이 필요합니다. 

2. **설명 요청 예시:**
   * **제한된 용어:**  "매출", "종목", "조건주문"과 같은 단어만 사용하는 것이 좋습니다. 
   * **구체적인 질문:**  특정 API 호출의 예시, 데이터 필드 의미, 구조 등을 명확히 표현합니다.

3. **구체적인 API 호출 예제:**
    *  주문 생성 시 특정 조건과 매개변수에 따른 예측이나 분석 요청 (예: "특정 종목의 주가 변동으로 인한 매수 주문 예측")


4. **AI 기반 추가 도움:** 
   * **기타 정보 제공:**  데이터 형식, 시스템 환경, API 호출 방법을 제공하면 더 정확한 분석 결과를 얻을 수 있습니다.

**추가적인 설명:** 
* 분석 결과 및 단계별 설명은 명확하고 구체적으로 작성해야 합니다. 
* 데이터 타입과 의미에 대한 자세한 설명을 포함하여 이해도를 높여야 합니다.
